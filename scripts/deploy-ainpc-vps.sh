...
if [ -z "$WINGS_TOKEN" ]; then
    echo "WINGS_TOKEN not set. Skipping server restart."
    echo "Set AINPC_WINGS_TOKEN env var to enable auto-restart."
    exit 0
fi
