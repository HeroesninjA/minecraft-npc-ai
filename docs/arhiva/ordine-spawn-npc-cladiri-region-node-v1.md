...
```text
ro.ainpc.spawn.HouseAllocation
ro.ainpc.spawn.HouseAllocation.ResidentPlan
ro.ainpc.spawn.HouseAllocationValidator
ro.ainpc.spawn.HouseAllocationValidationResult
```

Integrare curenta:

- `HouseAllocation` exporta metadata temporara pentru `WorldPlace`: `role=home`, `family_id`, `max_residents`, `residents`
- `HouseAllocation` produce determinist lista de `NpcSpawnPlan`
- validatorul verifica existenta casei, capacitatea, rezidenti duplicati, node-uri proprii de spawn/home/bed si work anchor pentru ocupatii reale
- validatorul verifica daca node-urile apartin place-urilor corecte si daca tipul/metadata lor este compatibil
...
- `NpcSpawnOrchestrator.validateHouseAllocation(...)` valideaza o casa cu rezidenti inainte de spawn
- `NpcSpawnOrchestrator.dryRunHouseAllocation(...)` valideaza `HouseAllocation` si `NpcSpawnPlan` fara entitati reale
- `NpcSpawnOrchestrator.spawnHousehold(...)` ruleaza batch-ul complet pentru o casa si apoi face family bind
...
- generatorul real nu creeaza inca automat `HouseAllocation` dintr-un plan de sat complet
...
- generatorul real trebuie sa alimenteze `HouseAllocation` din `BuildingPlan`/`SettlementPlan`
...
Stare: implementata initial la nivel de orchestrator, nu inca drept comanda admin dedicata.
...
- `NpcSpawnOrchestrator.dryRunHouseAllocation(...)` verifica `HouseAllocation`
- dry-run-ul produce `NpcSpawnPlan` si ruleaza validarea de mapping/world fara spawn real
- testele automate acopera conversia `HouseAllocation -> NpcSpawnPlan` si validari critice

Ce mai lipseste:

- comenzi admin dedicate pentru validarea unui plan incarcat din config/fisier
- format stabil de plan serializat pentru smoke test manual
...
Stare: implementata partial pentru batch-ul de household.
...
- `NpcSpawnOrchestrator.spawnHousehold(...)` face preflight complet inainte de spawn
- daca un spawn individual esueaza dupa ce alti NPC au fost creati, NPC-urile create in acel batch sunt sterse invers
- daca family binding esueaza, batch-ul incearca rollback peste NPC-urile create

Ce mai lipseste:

- tranzactie DB reala pentru toate scrierile asociate spawn-ului
- jurnal explicit de compensare pentru entitati/lume daca generatorul de cladiri esueaza
- rollback pentru constructii fizice sau patch-uri de sat, cand acestea vor exista

## Concluzie

Ordinea corecta este `plan -> constructie -> region -> place -> node -> alocare -> spawn NPC -> save -> family bind -> audit`.

Urmatorul pas concret nu mai este `HouseAllocation`; acesta exista si este conectat la `NpcSpawnPlan`.

Pentru MVP de spawn order, nucleul este acum acoperit: `HouseAllocation`, validare, dry-run, spawn batch, family bind, audit si rutina.

Pentru productie mai lipsesc:

- generatorul real care produce automat `HouseAllocation` din sat/cladiri/node-uri
- comenzi admin pentru planuri serializate si smoke test manual
