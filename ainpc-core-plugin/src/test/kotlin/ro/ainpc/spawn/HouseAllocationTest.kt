...
class HouseAllocationTest {
    ...
    fun toNpcSpawnPlansCarriesHouseFamilyAndResidentNodes() {
        ...
        assertEquals("father", plan.relationRole())
        assertEquals("artisan", plan.socialRole())
        assertEquals("adult", plan.ageGroup())
        assertEquals("day_worker", plan.routineProfile())
        assertEquals("giver", plan.questRole())
        assertEquals("ion_forge_01", plan.backstorySeed())
