package ro.ainpc.gui.screens

import org.junit.jupiter.api.Assertions.assertTrue
import org.junit.jupiter.api.Test
import java.io.File

class QuestCreatorScreensDiagnosticsTest {
    private val root = "src/main/kotlin/ro/ainpc/gui/screens/"

    @Test
    fun questCreatorGui() {
        val src = File(root + "QuestCreatorGui.kt").readText()
        assertTrue(src.contains("Quest Creator"))
        assertTrue(src.contains("AI Quick Actions"))
        assertTrue(src.contains("Quest AI create"))
        assertTrue(src.contains("Quest AI help"))
        assertTrue(src.contains("Quest AI refine selection"))
        assertTrue(src.contains("Quest Authoring"))
        assertTrue(src.contains("Quest Authoring read-only (seed, draft, validare)."))
        assertTrue(src.contains("Quest Create/Refine."))
        assertTrue(src.contains("Quest Draft Preview"))
        assertTrue(src.contains("Quest Draft Validation"))
        assertTrue(src.contains("Definitii"))
        assertTrue(src.contains("deschide quest authoring"))
        assertTrue(src.contains("Test Quest"))
        assertTrue(src.contains("Quest Editor"))
        assertTrue(src.contains("Creeaza Quest Nou"))
        assertTrue(src.contains("/ainpc quest create ai"))
        assertTrue(src.contains("/ainpc quest preview"))
        assertTrue(src.contains("/ainpc quest validate"))
        assertTrue(src.contains("ainpc quest create ai help"))
        assertTrue(src.contains("creator_quest_map_target"))
        assertTrue(src.contains("creator_quest_log_filter"))
        assertTrue(src.contains("objective:<key>"))
        assertTrue(src.contains("Target curent"))
        assertTrue(src.contains("Filtru curent"))
        assertTrue(src.contains("Query curent"))
    }

    @Test
    fun questCreatorDefinitionsGui() {
        val src = File(root + "QuestCreatorDefinitionsGui.kt").readText()
        assertTrue(src.contains("Definitii Progresie"))
        assertTrue(src.contains("progressionId"))
        assertTrue(src.contains("mechanicId"))
        assertTrue(src.contains("creator_defs_filter"))
        assertTrue(src.contains("Filtru definitii"))
        assertTrue(src.contains("GuiNavigation.addStandardControls"))
    }

    @Test
    fun questCreatorTestGui() {
        val src = File(root + "QuestCreatorTestGui.kt").readText()
        assertTrue(src.contains("Test Quest") || src.contains("Quest Test"))
        assertTrue(src.contains("quest_test_target"))
        assertTrue(src.contains("Force target"))
        assertTrue(src.contains("Quest Debug"))
        assertTrue(src.contains("Story Context"))
        assertTrue(src.contains("Quest Anchors"))
        assertTrue(src.contains("Quest Status"))
        assertTrue(src.contains("quest anchors \$target") || src.contains("ainpc quest anchors "))
    }

    @Test
    fun questEditGui() {
        val src = File(root + "QuestEditGui.kt").readText()
        assertTrue(src.contains("Editeaza Quest") || src.contains("Quest Edit"))
        assertTrue(src.contains("Cauta quest"))
        assertTrue(src.contains("quest_edit_query"))
        assertTrue(src.contains("Quest AI refine selection"))
        assertTrue(src.contains("Quest Creator"))
        assertTrue(src.contains("deschide quest creator"))
        assertTrue(src.contains("Quest Draft Preview"))
        assertTrue(src.contains("Quest Draft Validation"))
        assertTrue(src.contains("Quest Authoring"))
        assertTrue(src.contains("Deschide analiza read-only de story, mapping si progresie."))
        assertTrue(src.contains("ainpc quest create ai from selection"))
        assertTrue(src.contains("/ainpc quest preview"))
        assertTrue(src.contains("/ainpc quest validate"))
        assertTrue(src.contains("GuiKey.AUTHORING"))
        assertTrue(src.contains("Status: definitie gasita"))
        assertTrue(src.contains("Quest Negasit"))
    }

    @Test
    fun questCreateGui() {
        val src = File(root + "QuestCreateGui.kt").readText()
        assertTrue(src.contains("Creeaza Quest") || src.contains("Quest Create"))
        assertTrue(src.contains("Quest AI"))
        assertTrue(src.contains("Quest AI refine draft"))
        assertTrue(src.contains("ainpc quest create ai"))
        assertTrue(src.contains("/ainpc quest create ai"))
        assertTrue(src.contains("Descriere"))
        assertTrue(src.contains("Locatie NPC"))
        assertTrue(src.contains("Status: completabil"))
        assertTrue(src.contains("Lipsesc"))
        assertTrue(src.contains("Next Stage:"))
        assertTrue(src.contains("Exportul Include Anchor-ul NPC Chiar Daca Locatia Lipseste."))
        assertTrue(src.contains("openTextInput"))
        assertTrue(src.contains("quest_id"))
        assertTrue(src.contains("quest_mechanic"))
        assertTrue(src.contains("quest_base"))
        assertTrue(src.contains("quest_obj_type"))
        assertTrue(src.contains("quest_obj_count"))
        assertTrue(src.contains("quest_stage_"))
        assertTrue(src.contains("quest_reward_type"))
        assertTrue(src.contains("quest_dialog_type"))
        assertTrue(src.contains("quest_dialog_speaker"))
        assertTrue(src.contains("quest_dialog_text"))
        assertTrue(src.contains("quest_obj_dialog"))
        assertTrue(src.contains("quest_system_msg"))
        assertTrue(src.contains("quest_obj_target"))
        assertTrue(src.contains("quest_reward_value"))
        assertTrue(src.contains("quest_reward_count"))
        assertTrue(src.contains("quest_reward_event_payload"))
        assertTrue(src.contains("Quest Event"))
        assertTrue(src.contains("Story Event"))
        assertTrue(src.contains("isShiftClick"))
        assertTrue(src.contains("quest_obj_type"))
        assertTrue(src.contains("quest_base"))
        assertTrue(src.contains("Shift-click: ruleaza presetul urmator."))
        assertTrue(src.contains("quest_reward_value"))
        assertTrue(src.contains("quest_dialog_type"))
        assertTrue(src.contains("Shift-click: ruleaza valoarea preset urmatoare."))
        assertTrue(src.contains("quest_reward_event_scope"))
        assertTrue(src.contains("quest_reward_event_target"))
        assertTrue(src.contains("quest_reward_event_key"))
        assertTrue(src.contains("quest_reward_event_title"))
        assertTrue(src.contains("quest_dialog_speaker"))
        assertTrue(src.contains("Reset Obiectiv"))
        assertTrue(src.contains("Reset Recompensa"))
        assertTrue(src.contains("Reset Dialog"))
        assertTrue(src.contains("Reset All Draft"))
        assertTrue(src.contains("quest reset-objective"))
        assertTrue(src.contains("quest reset-reward"))
        assertTrue(src.contains("quest reset-dialog"))
        assertTrue(src.contains("quest reset-draft"))
        assertTrue(src.contains("ainpc quest accept nearest"))
        assertTrue(src.contains("Quest Draft Preview"))
        assertTrue(src.contains("Quest Draft Validation"))
        assertTrue(src.contains("Quest Editor"))
        assertTrue(src.contains("Quest Authoring"))
        assertTrue(src.contains("Status: completabil"))
        assertTrue(src.contains("Preview JSON"))
        assertTrue(src.contains("Validate Draft"))
        assertTrue(src.contains("Preview in chat"))
        assertTrue(src.contains("quest edit"))
        assertTrue(src.contains("authoring"))
        assertTrue(src.contains("Shift-click: ruleaza scope-ul preset urmator."))
        assertTrue(src.contains("Shift-click: ruleaza target-ul preset urmator."))
        assertTrue(src.contains("Shift-click: ruleaza event key preset urmator."))
        assertTrue(src.contains("Shift-click: ruleaza titlul preset urmator."))
        assertTrue(src.contains("Shift-click: ruleaza vorbitorul preset urmator."))
        assertTrue(src.contains("Exporta Draft JSON"))
        assertTrue(src.contains("Sugestii"))
    }
}
