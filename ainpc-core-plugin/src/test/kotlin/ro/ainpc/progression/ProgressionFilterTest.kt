...
class ProgressionFilterTest {
    @Test
    fun matchesDefinitionByStructuredMetadataFields() {
    ...
    @Test
    fun matchesStoredProgressionByStatusAndStructuredMetadataFields() {
        val progression = storedProgression()
    ...
    fun matchesDutyProgressionsByGenericMetadata() {
        val definition = ProgressionDefinition(
            ...
            "Rondul Strajerului", "Sarcina NPC de patrula.", "repeatable", "duty", "DUTY",
            "Sarcini NPC", "sarcina", "sarcini", 2, 3, 2, 2, true, true
        )
    ...
    @Test
    fun matchesBountyProgressionsByGenericMetadata() {
        val definition = ProgressionDefinition(
            "medieval:local_bounties:B01", "medieval", "local_bounties", "bounty", "B01", "medieval:B01", "B01",
            "Recompensa Drumului Vechi", "Bounty local de vanatoare.", "repeatable", "hunt", "BOUNTY",
            "Bounty locale", "bounty", "bounty-uri", 1, 3, 2, 2, true, true
        )
    ...
    @Test
    fun matchesEventProgressionsByGenericMetadata() {
        val definition = ProgressionDefinition(
            "medieval:village_events:E01", "medieval", "village_events", "event", "E01", "medieval:E01", "E01",
            "Alarma Fantanii din Piata", "Eveniment local din piata.", "repeatable", "event", "WORLD_EVENT",
            "Evenimente locale", "eveniment", "evenimente", 2, 3, 2, 2, true, true
        )
    ...
    @Test
    fun matchesTutorialProgressionsByGenericMetadata() {
        val definition = ProgressionDefinition(
            "medieval:onboarding:T01", "medieval", "onboarding", "tutorial", "T01", "medieval:T01", "T01",
            "Indrumarea Avizierului", "Tutorial de onboarding.", "side", "tutorial", "TUTORIAL",
            "Tutoriale", "tutorial", "tutoriale", 1, 3, 2, 2, false, true
        )
    ...
    @Test
    fun matchesRitualProgressionsByGenericMetadata() {
        val definition = ProgressionDefinition(
            "medieval:village_rituals:R01", "medieval", "village_rituals", "ritual", "R01", "medieval:R01", "R01",
            "Luminile Vechiului Altar", "Ritual local.", "repeatable", "ritual", "RITUAL",
            "Ritualuri locale", "ritual", "ritualuri", 1, 4, 3, 2, true, true
        )
    ...
    private fun definition(): ProgressionDefinition {
        return ProgressionDefinition(
            "medieval:village_contracts:C02", "medieval", "village_contracts", "contract", "C02", "medieval:C02", "C02",
            "Avizierul Pietei", "Contract de verificare a avizierului pietei.", "side", "investigation", "TRADE_DEAL",
            "Contracte de sat", "contract", "contracte", 3, 3, 2, 2, true, true
        )
    }

    private fun storedProgression(): StoredProgression {
