...
for line in r.stdout.splitlines():
    ...
    params = []
    if ps_raw.strip():
        for i, pt in enumerate(ps_raw.split(',')):
            pt = pt.strip()
            parts = pt.rsplit(' ', 1)
            pname = parts[1] if len(parts) == 2 else 'p' + str(i)
...
for m in methods:
    ...
    # Unit (void) methods: block body
    if not ret:
        out.append('    ' + vis + 'fun ' + name + '(' + ps + ') {')
        out.append('        TODO()')
        out.append('    }')
    # Typed methods: expression body with explicit return
    else:
