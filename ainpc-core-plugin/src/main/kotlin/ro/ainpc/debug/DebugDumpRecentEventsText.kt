...
object DebugDumpRecentEventsText {
    private val DATE_FORMAT: DateTimeFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")

    @JvmStatic
    fun buildRecentEventsText(plugin: AINPCPlugin): String {
        val sb = StringBuilder()
        sb.append("=== Recent Public Events (ultimele 50) ===\n\n")
        ...
        try {
            databaseManager.prepareStatement(sql).use { statement ->
                statement.executeQuery().use { resultSet ->
                    ...
                    if (count == 0) {
                        sb.append("Nu exista evenimente inregistrate.\n")
                    } else {
                        sb.append("Total: $count evenimente afisate.\n")
                    }
                }
            }
