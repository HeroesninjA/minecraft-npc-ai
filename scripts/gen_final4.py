...
# Pattern: optional[private/public/protected] optional[static] optional[final/synchronized] return_type name(params);
pat = r'^\s*(?:(private|public|protected)\s+)?(?:(static)\s+)?(?:(?:final|synchronized|native|abstract|strictfp)\s+)*(<[^>]+>\s+)?((?:[\w.<>\[\],\s])+?)\s+(\w+)\(([^)]*)\)\s*;'
methods = []
...
for line in r.stdout.splitlines():
    ...
    for i, pt in enumerate(split_params(ps_raw)):
        ...
        if pname is None: pname = 'p' + str(i)
...
def j2k(t):
    ...
    # Strip lowercase package prefixes only
    parts = t.split('.')
    result_parts = []
    i = 0
    while i < len(parts):
...
for m in methods:
    ...
    gen_str = (m['gen'] or '').strip()
    gen_prefix = (gen_str + ' ') if gen_str else ''
